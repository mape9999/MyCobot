...
def train_td3_mycobot_3d(target_position, timesteps, learning_rate, random_start):
    """Entrena el modelo TD3 en 3D (controlando 3 articulaciones)"""
    print("🚀 ENTRENAMIENTO 3D DE MYCOBOT con TD3")
    ...
    print(f"🔧 train_freq: {train_freq}, gradient_steps: {gradient_steps}, batch_size: {batch_size}, tau: {tau}, buffer_size: {buffer_size}")
    ...
    model = TD3(
        ...
        verbose=1,
        tensorboard_log="./td3_mycobot_improved_tensorboard/",
        policy_kwargs=dict(
            net_arch=[256, 256]
        )
    )

    print("🤖 Modelo TD3 3D creado")
    ...
    model.learn(
        ...
        progress_bar=True,
        tb_log_name=f"TD3_{int(time.time())}"
    )

    training_time = time.time() - start_time

    model_name = f"td3_mycobot_improved_3d_{timesteps}steps.zip"
    ...
    print("\n🎉 ENTRENAMIENTO 3D TD3 COMPLETADO!")
...
def test_td3_mycobot_3d(target_position, episodes, random_start, model_steps=None):
    """Prueba el modelo TD3 3D entrenado"""

    if model_steps is not None:
        model_path = f"td3_mycobot_improved_3d_{model_steps}steps.zip"
    else:
        model_files = glob.glob("td3_mycobot_improved_3d_*steps.zip")
        if not model_files:
            print("❌ No se encontraron modelos 3D TD3 entrenados")
            print("💡 Usa --mod-steps <número> para especificar el modelo")
            return
        model_files.sort(key=os.path.getmtime, reverse=True)
        model_path = model_files[0]
        match = re.search(r'td3_mycobot_improved_3d_(\d+)steps\.zip', model_path)
        if match:
            detected_steps = match.group(1)
            print(f"🔍 Modelo TD3 3D más reciente detectado: {model_path} ({detected_steps} steps)")

    print(f"🧪 PROBANDO MODELO TD3 3D: {model_path}")
    print(f"🎯 Objetivo: {target_position}")
...
def main():
    parser = argparse.ArgumentParser(description='Entrenar o probar MyCobot 3D con TD3')
    parser.add_argument('--train', action='store_true', help='Entrenar el modelo 3D con TD3')
    parser.add_argument('--test', action='store_true', help='Probar el modelo 3D TD3 entrenado')
